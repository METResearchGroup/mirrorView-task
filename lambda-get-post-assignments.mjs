...
const POST_ASSIGNMENTS_FILE = 'data/post_assignments.json';
...
export const handler = async (event) => {
    ...
    try {
        ...
        const { prolific_id, party_group, all_posts } = body;
        ...
        try {
            ...
            if (error.name === 'NoSuchKey') {
                ...
                console.log('Created new post assignments file');
            } else {
        ...
        console.log(`Available posts for ${party_group}: ${availablePosts.length}/${all_posts.length}`);
        ...
        console.log(`Assigned ${selectedPosts.length} posts to ${prolific_id} (${party_group}): ${selectedPosts.map(p => p.post_number).join(', ')}`);

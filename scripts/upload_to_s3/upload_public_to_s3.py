"""Stage public/ to s3_upload/<timestamp>/, validate config.js, upload to S3 one key at a time."""
...
def print_summary(
    release_dir: Path, manifest: dict[str, Any], *, dry_run: bool
) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage public/ and upload website assets to S3 (one object per file)."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Stage files and write manifest.json, but do not upload to S3.",
    )
    parser.add_argument(
    ...
    print_summary(release_dir, manifest, dry_run=args.dry_run)

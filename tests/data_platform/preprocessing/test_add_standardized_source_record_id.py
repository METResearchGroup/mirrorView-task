...
from data_platform.preprocessing.shared_columns import add_standardized_source_record_id
...
class TestAddStandardizedSourceRecordId:
    """Tests for add_standardized_source_record_id()."""
    ...
    def test_copies_reddit_comment_fullname_and_keeps_original(self) -> None:
        ...
        result = add_standardized_source_record_id(source, REDDIT_SPEC)

        assert result.iloc[0][STANDARDIZED_SOURCE_RECORD_ID_COLUMN] == comment_fullname
        assert result.iloc[0]["comment_fullname"] == comment_fullname
        assert STANDARDIZED_SOURCE_RECORD_ID_COLUMN not in source.columns
    ...
    def test_copies_twitter_tweet_id_and_keeps_original(self) -> None:
        ...
        result = add_standardized_source_record_id(source, TWITTER_SPEC)

        assert result.iloc[0][STANDARDIZED_SOURCE_RECORD_ID_COLUMN] == tweet_id
    ...
    def test_copies_bluesky_uri_and_keeps_original(self) -> None:
        ...
        result = add_standardized_source_record_id(source, BLUESKY_SPEC)

        assert result.iloc[0][STANDARDIZED_SOURCE_RECORD_ID_COLUMN] == uri
    ...
    def test_raises_when_reddit_comment_fullname_is_missing(self) -> None:
        ...
        with pytest.raises(KeyError):
            add_standardized_source_record_id(source, REDDIT_SPEC)

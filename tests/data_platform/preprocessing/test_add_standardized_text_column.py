...
from data_platform.preprocessing.runner import add_standardized_text_column
...
class TestAddStandardizedTextColumn:
    """Tests for add_standardized_text_column()."""
    ...
    def test_copies_reddit_body_onto_text_and_keeps_body(self) -> None:
        ...
        result = add_standardized_text_column(source, REDDIT_SPEC)

        assert result.iloc[0][STANDARDIZED_TEXT_COLUMN] == body
        assert result.iloc[0]["body"] == body
        assert STANDARDIZED_TEXT_COLUMN not in source.columns
    ...
    def test_keeps_existing_twitter_text(self) -> None:
        ...
        result = add_standardized_text_column(source, TWITTER_SPEC)

        assert result.iloc[0][STANDARDIZED_TEXT_COLUMN] == text
    ...
    def test_raises_when_original_platform_text_column_is_missing(self) -> None:
        ...
        with pytest.raises(KeyError):
            add_standardized_text_column(source, REDDIT_SPEC)

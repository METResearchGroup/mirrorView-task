...
def test_require_all_runs_complete_raises_when_root_missing(data_root) -> None:
...
def test_require_all_runs_complete_passes_when_sync_completed(data_root) -> None:
    storage = BlueskyStorageManager(StorageStage.RAW, VALID_DATASET_ID)
    write_stage_metadata(storage.create_new_run_dir("2026_01_01-00:00:00"))
    storage.require_all_runs_complete()


def test_require_all_runs_complete_raises_when_sync_incomplete(data_root) -> None:
    storage = BlueskyStorageManager(StorageStage.RAW, VALID_DATASET_ID)
...
def test_require_all_runs_complete_raises_when_metadata_missing(data_root) -> None:
    storage = BlueskyStorageManager(StorageStage.RAW, VALID_DATASET_ID)
    storage.create_new_run_dir("2026_01_01-00:00:00")
    with pytest.raises(RuntimeError, match="complete locally"):
        storage.require_all_runs_complete()
...
def test_require_all_runs_complete_passes_for_stage_without_sync_status(data_root) -> None:

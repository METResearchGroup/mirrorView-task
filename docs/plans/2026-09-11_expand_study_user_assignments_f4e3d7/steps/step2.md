...
- **Caller:** the same `run.py` `main` from Step 1, then `experiments/expand_study_user_assignments_2026_09_11/upload.py` `main`
- **Task:** Re-run the Step 1 pytest command, export AWS credentials, run the live remaining-labels command, prove the first 3879 source feeds match pull request 278, upload a new timestamped prefix under `jspsych-mirror-view-2026-09-09`, point the job YAML and lookup Lambda at that prefix, prove a returning participant still receives the same 20 posts, and commit `RESULTS.md`.
- **Out of scope:** Rewriting Step 1 modules except docstring fixes, adding files under the repo-root `tests/` folder, changing catalogs, changing the remaining labels CSV, overwriting `2026_09_09-23:06:02`, running `terraform apply`, resetting DynamoDB, editing the assignment-service repo.

## Files to inspect (read-only)

- `/workspace/experiments/expand_study_user_assignments_2026_09_11/run.py`
- `/workspace/experiments/expand_study_user_assignments_2026_09_11/README.md`
- `/workspace/experiments/expand_study_user_assignments_2026_09_11/constants.py`
...
- `/workspace/experiments/expand_study_user_assignments_2026_09_11/RESULTS.md` (written by the live command, then committed)
- `/workspace/experiments/expand_study_user_assignments_2026_09_11/README.md` only to add the live `batch_uri`, Democrat row count, and Republican row count
- `/workspace/experiments/expand_study_user_assignments_2026_09_11/upload.py` (new, if Step 1 did not already include it)
...
- `/workspace/experiments/expand_study_user_assignments_2026_09_11/tests/**`
...
Expected stdout includes `base_users=3879`, `extra_ten_ten=1000`, `user_count=4879`, `democrat_rows=2440`, `republican_rows=2439`, `assignment_slots=97580`, `extra_labels=20023`, `unused_remaining=0`, `s3_uri=s3://mirrorview-experimental-artifacts/experiments/expand_study_user_assignments_2026_09_11/study_user_assignments.csv`, and `csv_sha256=`.
...
expanded_key = (
    "experiments/expand_study_user_assignments_2026_09_11/"
    "study_user_assignments.csv"
)
...
new_d = pd.read_csv(
    Path(
        "experiments/expand_study_user_assignments_2026_09_11/batch/"
...
If either identity check fails, stop. Do not upload to the study bucket.
...
export AWS_ACCESS_KEY_ID="$LAB_AWS_ACCESS_KEY_ID"
export AWS_SECRET_ACCESS_KEY="$LAB_AWS_ACCESS_KEY_SECRET"
PYTHONPATH=. uv run python experiments/expand_study_user_assignments_2026_09_11/upload.py
```
...
Put that `batch_uri` into `jobs/config/mirrorview_2026_09_09.yaml` `assignment.batch_uri` and into `webapp/lambdas/lambda-get-post-assignments.mjs` `SEPTEMBER_BATCH_URI`. Do not change `STUDY_ITERATION_ID`. Do not run `terraform apply`.
...
aws lambda invoke \
  ...
  /tmp/expand_returning.json
```

Expected: the payload has 20 `assigned_post_ids`, and they match the original Democrat row `democrat-training_assisted-0001` from `2026_09_09-23:06:02`. DynamoDB `s3_key` for that user still points at the old prefix.
...
aws lambda invoke \
  ...
  --region us-east-2 \
  --cli-binary-format raw-in-base64-out \
  --payload '{"study_id":"mirrorview","study_iteration_id":"dev-mirrorview_2026_09_09","prolific_id":"manual-cli-expand-2026-09-11-d","political_party":"democrat","assignment_batch_uri":"s3://jspsych-mirror-view-2026-09-09/precomputed_assignments/<timestamp>"}' \
  /tmp/expand_new_dev.json
```
...
`RESULTS.md` records base users, extra mixed count, user count, Democrat rows, Republican rows, leftover-left counts, assignment slots, extra labels, unused remaining labels, source SHA-256, experimental S3 URI, new `batch_uri`, and a note that `2026_09_09-23:06:02` was left in place.

Add one `CHANGELOG.md` line under today's date. The line names 4879 feeds, 2440 Democrat rows, 2439 Republican rows, and the new prefix.

Copy the live command stdout to `/opt/cursor/artifacts/expand_study_user_assignments_run.log`. Copy the identity check stdout to `/opt/cursor/artifacts/expand_study_user_assignments_identity.log`.
...
- `RESULTS.md` records 4879, 2440, 2439, 20023 extra labels, and both prefixes.
...
- `terraform apply`.
- A new production assignment created only to test extras.

...
Run the ten-post smoke for `llm_toxicity_tiered` exactly once against the canonical campaign prefix. The smoke may run only once per feature, so the run is gated on an empty prefix.
...
| Label field | `toxicity_tier` (one of `low`, `medium`, `high`) |
| Feature prefix | `s3://mirrorview-experimental-artifacts/data_platform/data/bluesky/bluesky_7e2c4a91-3b5f-4d8e-a6c1-0f9b8d2e5a73/features/bluesky_2026_09_03_235130_llm_features_v1/llm_toxicity_tiered/` |
| Region | `us-east-2` |

## Files to inspect (read-only)

- `docs/plans/2026-09-05_generate_bluesky_llm_features_4d8a7c/steps/step14.md`
- `data_platform/generate_features/smoke_bluesky_campaign.py`

## Files allowed to change

None in this step. The smoke caller itself writes the Git copies under `docs/plans/2026-09-05_generate_bluesky_llm_features_4d8a7c/reports/smoke/llm_toxicity_tiered/`.

## Files forbidden to change

Everything under `data_platform/`, `lib/`, `ml_tooling/`, `tests/`, `pyproject.toml`, `CHANGELOG.md`, and every file in `docs/plans/2026-09-05_generate_bluesky_llm_features_4d8a7c/` other than the smoke report folder. Nothing under the S3 prefixes of the other six features, including `is_news_or_opinion/`, `is_political/`, `is_likely_spam/`, `is_self_contained/`, `is_structurally_complete/`, and `political_stance/`. Nothing under `is_toxic_tiered/`, and never run the Perspective feature `is_toxic_tiered`.
...
```text
smoke_prefix=s3://mirrorview-experimental-artifacts/.../llm_toxicity_tiered/smoke/
smoke_rows=10
...
cost_report=docs/plans/2026-09-05_generate_bluesky_llm_features_4d8a7c/reports/smoke/llm_toxicity_tiered/llm_toxicity_tiered_cost_report.json
```

# Run the Phase A ten-post smoke for llm_toxicity_tiered and post its cost estimate
...
An operator confirms the canonical S3 feature prefix is empty, runs the smoke caller once, confirms that only the four untagged `smoke/` objects exist, commits the three temporary review artifacts, and posts the cost estimate on issue #194.
...
flowchart TD
    ...
    C --> D[Compare output ids with deterministic_ten_post_ids.json and check that every toxicity_tier is low, medium, or high]
    D --> E[Commit three temporary smoke artifacts]
    E --> F[Post cost estimate on issue #194]
...
List the canonical feature prefix with boto3 and stop if any object exists. Then run the smoke caller exactly as `step14.md` shows, without a smoke prefix override.

### Step 2: Verify the S3 state and commit the temporary artifacts

List the prefix again, confirm the four untagged `smoke/` objects and zero `batches/` objects, confirm the ten output ids match the committed deterministic sample, and confirm every `toxicity_tier` value is one of `low`, `medium`, or `high`. Append the observed output to the S3 checks file and commit the three temporary artifacts.

### Step 3: Post the cost estimate and hand off to Phase B

Post the model, token averages, smoke cost, and estimated 200,000-post cost on issue #194. Push the branch and return the PR title and body to the epic manager. Record what Phase B must do after sign-off.

## What "done" looks like

1. The canonical prefix `llm_toxicity_tiered/` holds exactly four untagged objects under `smoke/` and nothing else.
2. The ten `source_record_id` values in `smoke/output.parquet` equal `reports/smoke/deterministic_ten_post_ids.json`.
3. Every `toxicity_tier` value in `smoke/output.parquet` is `low`, `medium`, or `high`.
...
5. The three temporary artifacts under `reports/smoke/llm_toxicity_tiered/` are committed, and no Parquet or CSV file is committed.
6. Issue #194 has a comment with the estimated full-run cost, or the PR body carries the same estimate when the token cannot comment on the issue.
7. The 200,000-post run has not started, and nothing exists under `is_toxic_tiered/`.

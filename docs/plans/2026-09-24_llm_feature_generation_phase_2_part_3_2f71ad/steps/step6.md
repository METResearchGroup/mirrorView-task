...
- `experiments/llm_feature_generation_phase_2_part_3_2026_09_24/outputs/shared/label/<run_timestamp>/`: label shards (`labels.jsonl`, resume index).
...
| `test_assemble_label_matrix_wide_format` | Shards for original + mirror | `assemble_label_matrix(...)` | Parquet has columns `post_id`, `text_surface`, `split`, `modal_decision`, and one column per `feature_id` (e.g. `cb_001`, not `cb_cb_001`); 18899 x 2 rows |
...
- `outputs/shared/label_matrix.parquet` has 37,798 rows and one column per codebook `feature_id` (e.g. `cb_001`, not `cb_cb_001`).

...
Issue 290 asks whether language models can match the keep/remove choices that September 2026 MirrorView participants made on 20 linked-fate post pairs. The live study is `mirrorview_2026_09_09` in bucket `jspsych-mirror-view-2026-09-09`. The work lives under `experiments/ai_simulation_responses_2026_09_11/`, and every derived file is also stored on S3 at that same path in bucket `mirrorview-experimental-artifacts`. Experiments 1 through 4 are prompt variants that share one cohort, one runner, and one 20-pair output schema. Experiment 5 only ranks errors on those labels. Experiment 6 repeats experiment 1 with one unnumbered pair per call and a yes/no answer, then compares those labels to experiment 1. No full model run starts until a smoke cost table is approved. Experiment 2 also waits on prompt-template approval. Experiment 6 waits on its own cost table under `experiment6/`.

## Happy flow

An operator downloads Prolific CSVs from the September bucket. The operator then confirms up to 1,000 complete participants in earliest-upload order and writes one shared cohort locally and on S3. Next the operator smokes 10 users on four models and writes a cost table at the same local path and S3 key. The operator then stops. After approval, four parallel Cursor Grok High subagents label the full cohort for experiments 1 through 4. A later command ranks 75 false-negative posts, 75 false-positive posts, and 100 lowest-F1 users, then uploads those lists to S3. After those scores exist, the operator smokes experiment 6 (10 users times 20 pairs on OpenAI, Nova Micro, and Qwen), writes `experiment6/COST_ESTIMATE.md`, and stops again. After written approval of that table, three Cursor Grok High subagents label the unique cohort, one model each. A later command scores experiment 6 and compares it to experiment 1 on the intersection of scored users.
...
- Experiment 6 setup, confirmed 2026-09-12. Reuse the shared cohort. Deduplicate by unique `prolific_id`, earliest session first. Do not run `--write-cohort` again. Do not download the September study bucket again. The live unique-user count is 998. Prompt content matches experiment 1: post pairs only, in stored `pair_order`. Each API call is a fresh prompt with one unnumbered pair. The user message is only `Post 1:` and `Post 2:` for that trial. It must not contain a pair number, must not say "of 20", and must not include the other 19 pairs. System prompt is `STUDY_SYSTEM_PROMPT_SINGLE_PAIR`. Model output is JSON `{"remove": "yes"}` or `{"remove": "no"}`, with those strings, not a boolean, not `1`/`0`, and not `remove_pair_indexes`. `yes` means remove both posts in the pair. `no` means keep both. Do not edit the experiment 1 through 4 system prompt string. After 20 successful pair calls for one user, map `yes` on study pair `k` onto the user-level remove list as 1-indexed index `k`. If any of a user's 20 pair calls fails, drop that user from scoring for that model. Do not impute `no`. Print `labeled_pairs`, `failed_pairs`, `scored_users`, and `failed_users`. Campaign grain is one row per user-pair. Record id is `{prolific_id}:{pair_index}`, parsed with `str.rsplit(":", 1)`. Expected successful rows per model are 998 times 20, which is 19,960. Feature name is `pair_yes_no`. Campaign id is `ai_simulation_responses_2026_09_11_experiment6`. Models are `openai` (`gpt-5.4-nano`), `bedrock_micro_nova` (`us.amazon.nova-micro-v1:0`), and `bedrock_qwen` (`qwen.qwen3-32b-v1:0`). Claude is excluded. Do not call `build_bedrock_engine`. Do not call `generate_campaign_feature`. Bedrock still uses `label_tasks_collecting_failures`, `max_tokens=256`, and concurrency 8. Smoke 10 unique cohort users (200 pair calls per model) under `experiment6/outputs/{model}/smoke/`. Write `experiment6/COST_ESTIMATE.md` locally and on S3. Stop. Full `--model` labeling requires `experiment6/APPROVAL.md`. Scoring reuses the experiment 1 metric tables and adds predicted remove rate (share of scored pairs the model removed). Compare experiment 6 to experiment 1 on the intersection of users scored in both experiments for that model. Required compare rows: user-level F1, post-level F1, accuracy, precision, recall, predicted remove rate, and the human gold remove rate on that intersection. Also report the share of user-pairs where experiment 1 and experiment 6 agree, and within-user variance of per-pair correctness for both experiments. Do not rewrite experiment 1 `RESULTS.md`. Do not add significance tests. Do not compare Claude.

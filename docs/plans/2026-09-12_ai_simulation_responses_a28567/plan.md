# Predict September 2026 keep/remove choices across six experiments
...
Issue 290 asks whether language models can match the keep/remove choices that September 2026 MirrorView participants made on 20 linked-fate post pairs. The live study is `mirrorview_2026_09_09` in bucket `jspsych-mirror-view-2026-09-09`. The work lives under `experiments/ai_simulation_responses_2026_09_11/`, and every derived file is also stored on S3 at that same path in bucket `mirrorview-experimental-artifacts`. Experiments 1 through 4 are prompt variants that share one cohort, one runner, and one 20-pair output schema. Experiment 5 only ranks errors on the experiment 1 through 4 labels. Experiment 6 repeats experiment 1 with one unnumbered pair per call and a yes/no answer, then compares the one-pair labels to experiment 1. No full model run starts until a smoke cost table is approved. Experiment 2 also waits on prompt-template approval. Experiment 6 waits on its own cost table under `experiment6/`.

## Happy flow

An operator downloads Prolific CSVs from the September bucket. The operator then confirms up to 1,000 complete participants in earliest-upload order and writes one shared cohort locally and on S3. Next the operator smokes 10 users on four models and writes a cost table at the same local path and S3 key. The operator then stops. After approval, four parallel Cursor Grok High subagents label the full cohort for experiments 1 through 4. A later command ranks 75 false-negative posts, 75 false-positive posts, and 100 lowest-F1 users, then uploads the lists to S3. After experiment 5 scores exist, the operator smokes experiment 6 on 10 users times 20 pairs for OpenAI, Nova Micro, and Qwen. The operator writes `experiment6/COST_ESTIMATE.md` and stops again. After written approval of the experiment 6 cost table, three Cursor Grok High subagents label the unique cohort, one model each. A later command scores experiment 6 and compares it to experiment 1 on the intersection of scored users.
...
flowchart TD
    ...
    J --> K[Smoke experiment 6 on 3 models]
    K --> L[Write experiment6 COST_ESTIMATE.md]
    L --> M{User approves experiment 6 cost}
    M -->|no| STOP6[Stop]
    M -->|yes| N1[Experiment 6 OpenAI]
    M -->|yes| N2[Experiment 6 Nova Micro]
    M -->|yes| N3[Experiment 6 Qwen]
    N1 --> P[Score experiment 6 and compare to experiment 1]
    N2 --> P
    N3 --> P
    P --> Q[Write experiment6 RESULTS.md locally and on S3]
```

## Approach

Reuse the OpenAI Batch engine and the Bedrock Converse helpers that the original-versus-mirror separability experiment already uses. Do not copy the engines four times. One shared runner takes an experiment number and a model name. Experiments 1 through 4 only change the extra context block in the user prompt. Experiment 6 keeps the experiment 1 posts-only content, but it sends one pair per call and stitches 20 yes/no answers into the same remove-list shape. Keep gold labels and predictions as separate parquet objects. Store every derived file in bucket `mirrorview-experimental-artifacts` with an object key that equals the local path, matching the other September 2026 experiments. Treat experiments 1 through 4 as prompt ablations on the same people. They are not a causal test of demographics or reflection. Treat experiment 6 as a process-match ablation of experiment 1. Report user-level means and post-level scores separately, because the two aggregations are not the same number. Simplicity and persona notes are in `reviews.md`.

## Decisions

- Create `experiments/ai_simulation_responses_2026_09_11/` with `shared/` plus `experiment1` through `experiment6`. Shared code lives in `shared/`. Each of experiments 1 to 4 has `README.md`, `SETUP.md`, `RESULTS.md`, and `outputs/{openai,bedrock_micro_nova,bedrock_qwen,bedrock_claude}/`. Experiment 5 has no model labels. Experiment 5 does have ranked CSVs under `experiment5/outputs/`. Experiment 6 has `README.md`, `SETUP.md`, `RESULTS.md`, `COST_ESTIMATE.md`, `APPROVAL.md`, and `outputs/{openai,bedrock_micro_nova,bedrock_qwen}/`. Claude has no experiment 6 objects.
...
- Parent `experiments/ai_simulation_responses_2026_09_11/README.md` is two sentences that name issue 290 and point at each experiment folder. The parent README is agent read-only after it is written, so later experiment 6 work must not edit it. Each experiment README is one or two sentences for the ablation question, then a redirect to that folder's `SETUP.md` and `RESULTS.md`. Start every README with the agent read-only banner from `experiments/filter_posts_used_for_stimulus_dataset_2026_09_08/README.md`.
...
- Instruction text is the training_assisted copy in `webapp/public/main.js` (the paragraph that starts "We are developing a new social media platform" and the political-mirror example). For experiments 1 through 4, the task change from the website is that the model sees all 20 pairs at once and returns remove indexes instead of clicking Allow Both or Remove Both on each trial. State in those `SETUP.md` files that humans judged one pair at a time and the model sees the full set in one prompt. Experiments 1 through 4 stay one call per user. Experiment 6 is the later one-pair process match against experiment 1 only. Do not add per-pair calls to experiments 2 through 4.
...
- Experiment 5 uses existing labels and existing post and user fields only. Rank posts by false-negative rate across models and experiments 1 to 4 (model said keep, gold is remove) and take the top 75. Rank posts by false-positive rate (model said remove, gold is keep) and take the top 75. Complementary errors are the split, so the 150-post set is 75 false negatives plus 75 false positives. Compare toxicity and stance shares in that set of 150 against the rest of the cohort posts. Rank users by mean user-level F1 across models and experiments 1 to 4, lowest first, and take 100. Describe those users with `party_group`, `political_ideology`, `age`, `education`, and mean gold remove rate. Within-user variance is the variance of per-pair correctness on experiment 1. Report summary statistics of that variance across users, one row per model. Across-model variance is the standard deviation of the four models' experiment 1 remove predictions on each user-pair. Report summary statistics of those standard deviations. Required ranked files, local and on S3, are `experiment5/outputs/false_negative_posts.csv`, `experiment5/outputs/false_positive_posts.csv`, and `experiment5/outputs/lowest_f1_users.csv`. `experiment5/RESULTS.md` is also required locally and on S3. Do not fold experiment 6 labels into experiment 5 ranks.
- Experiment 6 setup, confirmed 2026-09-12. Reuse the shared cohort. Deduplicate by unique `prolific_id`, earliest session first. Do not run `--write-cohort` again. Do not download the September study bucket again. The live unique-user count is 998.
- Experiment 6 prompt content matches experiment 1. Post pairs only, in stored `pair_order`. Each API call is a fresh prompt with one unnumbered pair. The user message is only `Post 1:` and `Post 2:` for that trial. It must not contain a pair number, must not say "of 20", and must not include the other 19 pairs.
- Experiment 6 system prompt is `STUDY_SYSTEM_PROMPT_SINGLE_PAIR`. Model output is JSON `{"remove": "yes"}` or `{"remove": "no"}`, with those strings. Do not use a boolean, `1`/`0`, or `remove_pair_indexes` as the model output field. `yes` means remove both posts in the pair. `no` means keep both. Do not edit the experiment 1 through 4 system prompt string.
- After 20 successful pair calls for one user, map `yes` on study pair `k` onto the user-level remove list as 1-indexed index `k`. If any of a user's 20 pair calls fails, drop that user from scoring for that model. Do not impute `no`. Print `labeled_pairs`, `failed_pairs`, `scored_users`, and `failed_users`.
- Experiment 6 campaign grain is one row per user-pair. Record id is `{prolific_id}:{pair_index}`, parsed with `str.rsplit(":", 1)`. Successful rows per model are 998 times 20, which is 19,960. Feature name is `pair_yes_no`. Campaign id is `ai_simulation_responses_2026_09_11_experiment6`.
- Experiment 6 models are `openai` (`gpt-5.4-nano`), `bedrock_micro_nova` (`us.amazon.nova-micro-v1:0`), and `bedrock_qwen` (`qwen.qwen3-32b-v1:0`). Claude is excluded. Do not call `build_bedrock_engine`. Do not call `generate_campaign_feature`. Bedrock still uses `label_tasks_collecting_failures`, `max_tokens=256`, and concurrency 8.
- Smoke experiment 6 on 10 unique cohort users (200 pair calls per model) under `experiment6/outputs/{model}/smoke/`. Write `experiment6/COST_ESTIMATE.md` locally and on S3. Stop. Full `--model` labeling requires `experiment6/APPROVAL.md`.
- Experiment 6 scoring reuses the experiment 1 metric tables and adds predicted remove rate (share of scored pairs the model removed). Compare experiment 6 to experiment 1 on the intersection of users scored in both experiments for that model. Required compare rows are user-level F1, post-level F1, accuracy, precision, recall, predicted remove rate, and the human gold remove rate on that intersection. Also report the share of user-pairs where experiment 1 and experiment 6 agree, and within-user variance of per-pair correctness for both experiments. Do not rewrite experiment 1 `RESULTS.md`. Do not add significance tests. Do not compare Claude.
- After cost approval, launch four Cursor Grok High subagents in parallel, one per experiment 1 to 4. Each subagent runs all four models for its experiment and does not score. Scoring is Step 4. Experiment 5 starts only when all 16 `final.parquet` objects exist. Do not implement a plugin registry or a prompt-strategy interface. Experiment 6 adds a yes/no schema next to the remove-index schema.
- Pytest lives under `experiments/ai_simulation_responses_2026_09_11/shared/tests/` and must not call OpenAI, Bedrock, or S3. Cover cohort completeness, pair-order rendering, 1-indexed expansion, the metric tables, the mirrored S3 key helper, one-pair rendering, `yes`/`no` parsing, stitch, failed-pair user drop, and Claude rejection. The usual "experiments skip pytest" line in `UNIT_TESTING_STANDARDS.md` does not apply, because a 0-based index bug would invalidate every score.
- Do not edit `data_platform/`, `webapp/`, `scripts/export_study_results.py`, `lib/constants.py`, `shared/flip_generation/`, or files under repo-root `tests/`. Do not upload to the study website bucket. Do not add demographics, reflection, Claude, or per-pair calls for experiments 2 through 4. `CHANGELOG.md` waits until a live full run has `RESULTS.md` files.
...
`N` is 1 to 4 for 20-pair labels and 1 to 6 for `RESULTS.md`. For experiments 1 to 4, `{model}` is `openai`, `bedrock_micro_nova`, `bedrock_qwen`, or `bedrock_claude`. For experiment 6, `{model}` is `openai`, `bedrock_micro_nova`, or `bedrock_qwen`. Git-only files (`*.py`, tests, `README.md`, `SETUP.md`) are not uploaded. Parquet, campaign objects, and experiment 5 CSVs stay out of git. Parent `COST_ESTIMATE.md`, `experiment6/COST_ESTIMATE.md`, and each `RESULTS.md` are committed after the live write, and they are also on S3. Do not overwrite the parent cost file when writing the experiment 6 cost file.
...
Read the 16 final parquets. Write the 75 false-negative posts, the 75 false-positive posts, the 100-user list, within-user variance, and across-model variance into `experiments/ai_simulation_responses_2026_09_11/experiment5/RESULTS.md`. Upload that report and the three ranked CSVs to S3. Do not read experiment 6 labels. See [steps/step5.md](steps/step5.md).

### Step 6: Repeat experiment 1 with one pair per prompt, then compare

Reuse the existing unique cohort users. Add the one-pair yes/no prompt, pair-level labeling grain, and stitch helper. Smoke 10 users times 20 pairs on OpenAI, Nova Micro, and Qwen. Write `experiment6/COST_ESTIMATE.md` and stop. After written approval, label the full unique cohort on those three models. Score experiment 6 with the experiment 1 tables plus predicted remove rate, then compare against existing experiment 1 `RESULTS.md` numbers on the per-model user intersection. See [steps/step6.md](steps/step6.md).

## What "done" looks like

1. `experiments/ai_simulation_responses_2026_09_11/` has `shared/`, six experiment folders, parent `README.md`, and parent `COST_ESTIMATE.md` after Step 2. Experiment 6 also has its own `COST_ESTIMATE.md` after Step 6 smoke.
...
8. After Step 6, `experiment6/RESULTS.md` exists locally and on S3 with user-level, post-level, party, toxicity, and stance tables, predicted remove rate, and a comparison against experiment 1 on the per-model user intersection for OpenAI, Nova, and Qwen only. Claude has no experiment 6 objects. Experiment 1 `RESULTS.md` and parent `COST_ESTIMATE.md` are unchanged by Step 6.
9. `PYTHONPATH=. uv run pytest experiments/ai_simulation_responses_2026_09_11/shared/tests -q` exits 0. Product engines, the website, and the September study bucket objects are unchanged.
...
After experiment 1 `RESULTS.md` exists, experiment 6 smoke and cost:

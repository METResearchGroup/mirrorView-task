...
Each user should see 20 posts. The team decision is to keep a 1:1 left/right split for as many users as possible, even if the remaining users see only left posts. As many feeds as possible stay 10 left and 10 right. The rest are 20 left and 0 right, because only left posts still need labels after the 10:10 feeds have taken every remaining right label. On the pull request 279 file, 10:10 count is 3,202. Leftover left remaining after those feeds is 13,522. Left-only count is 677. User count is 3,879 and post slots are 77,580. Extra labels are 23. Every remaining label is assigned.
...
- There are two feed kinds only. A 10:10 feed has 10 left posts and 10 right posts. A left-only feed has 20 left posts and 0 right posts. Do not write 11:9 or 12:8 feeds. Count 10:10 feeds as the ceiling of right remaining divided by 10. Count left-only feeds as the ceiling of leftover left remaining after those 10:10 feeds, divided by 20. Leftover left remaining equals left remaining minus 10 times the 10:10 count. On the pull request 279 file, 10:10 count is 3,202, leftover left remaining is 13,522, and left-only count is 677. User count is 3,879. Post slots are 77,580. Extra labels are 23, which is 5 extra right labels and 18 extra left labels. Do not cap at 3,878 users, because 3,202 feeds at 10:10 plus 676 left-only feeds would leave 2 leftover left remaining unused. Fill every 10:10 feed first, then every left-only feed. Do not write a partial user.
- Recipe 1 preferred counts, in cell order 1 through 6: 2, 5, 3, 2, 5, 3. Recipe 2 preferred counts: 3, 5, 2, 3, 5, 2. Odd 10:10 users use recipe 1 as the preferred split. Even 10:10 users use recipe 2. Left-only users use a doubled left half of the same recipes. Odd left-only users prefer 4, 10, 6 from cells 1, 2, 3. Even left-only users prefer 6, 10, 4 from cells 1, 2, 3. Preferred counts are the starting target. When remaining in a cell runs out, steal within the same party.

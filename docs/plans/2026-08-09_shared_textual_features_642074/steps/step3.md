...
Move the three LLM classifiers into `shared/textual_features/{valence,intergroup,prime}.py` with the same prompts, structured-output models, and `classify_post` / `classify_texts` behavior as today’s experiment `classifier.py` files. Register them in the catalog. Do **not** move MirrorView CSV loading or label CSV writes into shared.

## Caller / unit of work

**Main caller (this step):** shared classify helpers (imported later by experiment wrappers in Step 4).

1. `classify_post(post: str)` → pydantic result (`ValenceClassification` / `IntergroupClassification` / `PrimeClassification`).
2. `classify_texts(posts: list[str])` → list of those results (tqdm progress as today).
3. `get_feature(VALENCE|INTERGROUP|PRIME).classify_post` is the same callable (or a thin bound wrapper).

**Out of scope:** rewriting prompts; four-way PRIME expansion; CSV `__main__` paths; live production labeling runs; experiment migration (Step 4).

## Files to inspect (read-only)

| Path | Why |
|------|-----|
| `/workspace/experiments/mirrors_content_analysis_2026_04_24/analysis/valence_classifier/classifier.py` | Prompt, model, classify helpers, CSV I/O to leave behind |
| `/workspace/experiments/mirrors_content_analysis_2026_04_24/analysis/intergroup_classifier/classifier.py` | Same |
| `/workspace/experiments/mirrors_content_analysis_2026_04_24/analysis/prime_classifier/classifier.py` | Same; binary any-of PRIME definition |
...
- `/workspace/shared/textual_features/__init__.py` (optional re-exports of classify helpers / result models)
- `/workspace/shared/textual_features/tests/test_registry.py` (assert classifier entries resolve and expose `classify_post`; do **not** require live API)
...
- Deterministic metric modules completed in Step 2 (unless a registry-only edit is required)
...
- Full system/human prompt strings (including few-shot examples).
- Structured-output field names (`is_positive`, `is_intergroup`, `is_prime`).
- LangChain `ChatPromptTemplate` + `with_structured_output` call pattern.
...
Remain a **single binary** any-of (prestige / in-group / moral / emotional). Do not add per-cue outputs.
...
# Expected: all deterministic + registry tests PASS; no live LLM required
...
**Pass when:**
1. Shared valence/intergroup/prime modules expose the frozen classify API and result models.
2. Prompts and field names match the experiment sources.
3. Registry classifier entries resolve with `kind="classifier"` and callable `classify_post`.
4. No CSV I/O or `Dataloader` usage inside `shared/textual_features/`.
5. Deterministic tests from Step 2 still pass.

**Fail when:** prompts are rewritten; PRIME is split into four classifiers; label CSV writers move into shared; experiment classifier files are migrated early (that is Step 4); step depends on a mandatory live OpenAI call.

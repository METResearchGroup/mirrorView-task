...
Show that the smoke ran once, wrote only the four untagged `smoke/` objects, labeled the shared deterministic ten posts with boolean values, and wrote nothing under `batches/`. Commit the three temporary Git artifacts that `step9.md` names.
...
- `docs/plans/2026-09-05_generate_bluesky_llm_features_4d8a7c/reports/smoke/is_political/is_political_s3_checks.txt`
...
Any Parquet or CSV file. Everything under `data_platform/`, `lib/`, `ml_tooling/`, `tests/`, `pyproject.toml`, `CHANGELOG.md`, `plan.md`, and `steps/*.md` of the epic plan folder. The `is_news_or_opinion/` smoke artifacts.
...
KEY = ("data_platform/data/bluesky/bluesky_7e2c4a91-3b5f-4d8e-a6c1-0f9b8d2e5a73/features/"
       "bluesky_2026_09_03_235130_llm_features_v1/is_political/smoke/output.parquet")
...
print(f"is_political_dtype={frame['is_political'].dtype}")
print(f"all_labels_boolean={str(all(isinstance(v, bool) for v in frame['is_political'].tolist())).lower()}")
print(f"is_political_counts={frame['is_political'].value_counts().to_dict()}")
PY
```

Expected: the Q44 columns `source_record_id`, `run_id`, `batch_id`, `request_id`, `attempt_count`, `label_timestamp`, `is_political`, then `deterministic_ids_match=true`, then `all_labels_boolean=true`, then counts that use only `True` and `False`.

### Record the observations

Append the listing before, the listing after, and the id and label check output to `is_political_s3_checks.txt` under a heading that names each command. Keep the caller's own check lines at the top of the file unchanged.

### Commit

```bash
git add docs/plans/2026-09-05_generate_bluesky_llm_features_4d8a7c/reports/smoke/is_political/is_political_cost_report.json \
        docs/plans/2026-09-05_generate_bluesky_llm_features_4d8a7c/reports/smoke/is_political/is_political_resume_evidence.json \
        docs/plans/2026-09-05_generate_bluesky_llm_features_4d8a7c/reports/smoke/is_political/is_political_s3_checks.txt
git commit -m "Record the is_political Phase A smoke evidence (issue #189)"
```

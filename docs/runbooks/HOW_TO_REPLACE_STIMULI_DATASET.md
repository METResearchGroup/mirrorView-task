...
- **Browser fetch path (deployed)**: `webapp/public/main.js` → `STUDY_SPEC.postCatalogPath` = `img/flips_scaled_2026_06_18.csv`
- **Upload allowlist / critical keys**: `webapp/scripts/upload_to_s3/constants.py` includes `img/flips_scaled_2026_06_18.csv`
...
You only need to replace the catalog CSV that gets uploaded under `webapp/public/` (or repo-root fallback; see below).
...
- `webapp/lambdas/lambda-get-post-assignments.mjs` hardcodes which assignment batch URI to use (see `resolveAssignmentBatchUri()` and `SCALED_BATCH_URI`).
...
- Update:
  - `webapp/lambdas/lambda-get-post-assignments.mjs` to point to the new `SCALED_BATCH_URI` (or equivalent)
...
The deploy pipeline uploads files from `webapp/public/` (and also supports repo-root fallbacks for allowlisted paths).

For the scaled run, the deployed key must be:

- `img/flips_scaled_2026_06_18.csv`

### Preferred: place under `webapp/public/`
...
This is mainly useful if you intentionally keep stimuli out of `webapp/public/` in git, but still want the upload tooling to pick it up.
...
- Confirm `webapp/lambdas/lambda-get-post-assignments.mjs` is pointing at the new batch URI.
...
```bash
bash webapp/scripts/upload_to_s3/verify_s3_object_matches_local.sh \
  --bucket jspsych-mirror-view-4 \
  --key img/flips_scaled_2026_06_18.csv \
  --local webapp/public/img/flips_scaled_2026_06_18.csv \
  --region us-east-2
```

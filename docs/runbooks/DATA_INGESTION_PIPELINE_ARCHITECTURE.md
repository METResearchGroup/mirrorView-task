...
Do not commit live PRAW or API sync artifacts under `data_platform/data/`. The promoted Reddit dump dataset at `data_platform/data/reddit/reddit_3d8a2c41-9b17-4e6f-a5d0-8c1b2e4f6079/` is the exception: its raw and preprocessed parquet files are stored with Git LFS. Promote those dump files from `data_platform/ingestion/data_dumps/reddit/pushshift_dump.yaml` with `PYTHONPATH=. uv run python data_platform/ingestion/data_dumps/reddit/promote_to_raw.py`.

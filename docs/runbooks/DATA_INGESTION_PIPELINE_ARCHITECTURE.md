...
Do not commit live PRAW or API sync files under `data_platform/data/`. The Reddit dump dataset at `data_platform/data/reddit/reddit_3d8a2c41-9b17-4e6f-a5d0-8c1b2e4f6079/` is allowed, and its raw and preprocessed parquet files are stored with Git LFS. To copy the dump parquet files into the raw runs, run `PYTHONPATH=. uv run python data_platform/ingestion/data_dumps/reddit/promote_to_raw.py`. The script reads `data_platform/ingestion/data_dumps/reddit/pushshift_dump.yaml`.

...
Omit `--checkpoint` when you run `generate_*_features.py` to create one `features/<timestamp>/` feature run folder. If an unfinished feature run already exists, you get an error. After an interrupt, pass `--checkpoint` with the feature run timestamp to keep writing into that unfinished feature run. You cannot continue a completed feature run.
...
Do not commit live PRAW or API sync files under `data_platform/data/`. Dump datasets are allowed, and their raw and preprocessed parquet files are stored with Git LFS. The dated Mirrorview Twitter runs (`mirrorview_2026-09-05.yaml` and `mirrorview_2026-09-07.yaml`) are also allowed. Their raw and preprocessed `posts.csv` files are stored with Git LFS. To copy Bluesky dump parquet into the pipeline raw path, run `PYTHONPATH=. uv run python data_platform/ingestion/data_dumps/bluesky/publish_dump_to_raw.py`. To copy Reddit dump parquet into the pipeline raw path, run `PYTHONPATH=. uv run python data_platform/ingestion/data_dumps/reddit/publish_dump_to_raw.py`. Preprocess those datasets with `--config data_platform/preprocessing/configs/bluesky/jetstream_dump.yaml` or `--config data_platform/preprocessing/configs/reddit/pushshift_dump.yaml`.
...
Shared logic is in `data_platform/preprocessing/runner.py`. Preprocessing reads the latest complete raw run, or all raw runs per platform rules. It validates and cleans text, then deduplicates. Output goes to `preprocessed/<timestamp>/posts.csv`, or platform-specific filenames. Preprocessing calls `StorageManager.require_all_runs_complete`, and it does not start if a raw sync is still in progress.
...
Continue an unfinished feature run with `--checkpoint`.

```bash
PYTHONPATH=. uv run python data_platform/generate_features/generate_bluesky_features.py \
  --dataset-id bluesky_c0ffee00-0000-4000-8000-000000000100 --checkpoint <timestamp>
```

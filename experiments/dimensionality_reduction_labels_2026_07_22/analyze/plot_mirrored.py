"""PCA/LDA single-scatter for mirrored-post Titan embeddings vs human keep/remove.
...
def main() -> int:
    if not X_MIRRORED_PATH.is_file():
        raise FileNotFoundError(f"Missing {X_MIRRORED_PATH} — run build_table.py first")

    X = np.load(X_MIRRORED_PATH)
    meta = load_meta()
    if len(meta) != X.shape[0]:
        raise ValueError(f"Row mismatch: meta={len(meta)} X={X.shape}")

    y = meta["label"].to_numpy()
    print(f"Loaded X_mirrored={X.shape} keep={int((y == 0).sum())} remove={int((y == 1).sum())}")
    ...
    pca_path = MIRRORED_DIR / "pca_remove_vs_keep.png"
    lda_path = MIRRORED_DIR / "lda_remove_vs_keep.png"

    pca_extra = plot_pca_single(
        Z_pca,
        y,
        pca_path,
        title="PCA (2D) of mirrored Titan — human remove vs keep",
    ...
    lda_extra = plot_lda_single(
        ...
        y,
        lda_path,
        title="LDA view of mirrored Titan — human remove vs keep",
    )
    ...
    emb_path = MIRRORED_DIR / "embeddings_2d.csv"
    emb.to_csv(emb_path, index=False)

    summary = {
        "feature_set": FEATURE_SET_MIRRORED,
    ...
    summary_path = MIRRORED_DIR / "reduction_summary.json"

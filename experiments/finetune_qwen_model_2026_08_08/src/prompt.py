"""Vendored keep/remove rubric prompt for Qwen LoRA fine-tuning.

Copied from the prompt-engineering experiment template; closing line asks for
``keep`` or ``remove`` only (not ``Allow Or Remove?``). Do not import
``experiments.llm_prompt_engineering_*``.

Run from root: PYTHONPATH=. uv run python experiments/finetune_qwen_model_2026_08_08/src/prompt.py
"""
...
def generate_user_prompt(post_1_text: str, post_2_text: str) -> str:
    """Fill the vendored rubric template for one original/mirror pair.

    Parameters
    ----------
    post_1_text
        Original post text (Post 1).
    post_2_text
        Mirror post text (Post 2).

    Returns
    -------
    str
        User-turn content ending with a keep/remove instruction.
    """
    return STUDY_PROMPT_TEMPLATE.format(
        ADD_KEEP_REMOVE_FEATURES_ADDENDUM=KEEP_REMOVE_FEATURES_ADDENDUM,

...
request 54 adapter (lean copy) and writes preds under preds/unanimous_lora/.
...
def _load_sagemaker_role_arn() -> str:
    """Return SAGEMAKER_ROLE_ARN from the environment.

    Prefer the Qwen exec role used by prior teachability runs.
    """
    role = os.environ.get("SAGEMAKER_ROLE_ARN", "").strip()
    if not role or role.startswith("[REDACTED]"):
        raise ValueError(
            "SAGEMAKER_ROLE_ARN is required "
            "(expected the mirrorview-qwen-finetune-sm-exec role ARN)."
        )
    if role.endswith("modernbert-sagemaker-execution"):
        raise ValueError(
            "SAGEMAKER_ROLE_ARN points at the ModernBERT role; "
            "set it to the mirrorview-qwen-finetune-sm-exec role ARN."
        )
    if "modernbert-sagemaker-executio" in role:
        raise ValueError(
            "SAGEMAKER_ROLE_ARN points at the ModernBERT role; "
            "set it to the mirrorview-qwen-finetune-sm-exec role ARN."
        )
...
def submit_job(config: prior.JobConfig, wait: bool) -> str:
    ...
    New SageMaker accounts cannot enable Debugger, so CreateTrainingJob
    fails unless debugger_hook_config=False and disable_profiler=True.
    """
    import sagemaker

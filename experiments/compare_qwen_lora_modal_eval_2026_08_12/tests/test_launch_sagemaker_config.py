...
Pseudocode
----------
given build_job_config for unanimous adapter infer
when the config is returned
then adapter_s3_uri ends with adapters/unanimous_passrole_probe3_lean
and output_s3_uri / PREDS_S3_URI end with preds/unanimous_lora
and image_uri contains mirrorview-larger_finetune_qwen_model_2026_08_08
and container_arguments equal ["infer_adapter"]
"""
...
def test_build_job_config_points_at_unanimous_adapter_and_preds() -> None:
    ...
    config = build_job_config(
        run_id="compare_dry",
        role_arn="arn:aws:iam::123456789012:role/test-sagemaker-exec",
        hf_token="hf_test",
        image_uri=(

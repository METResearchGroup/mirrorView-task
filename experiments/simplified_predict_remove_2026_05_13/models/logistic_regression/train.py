"""Train logistic regression baseline on embeddings + categorical metadata."""
...
from experiments.simplified_predict_remove_2026_05_13.features import (
    ...
    EmbeddingMetadataMatrixBuilder,
    JOIN_COL_MIRROR,
    JOIN_COL_ORIGINAL,
    classification_metrics_summary,
    join_embeddings,
    load_embeddings_via_dynamodb_and_s3,
)
...
def build_xy_from_joined(df: pd.DataFrame, mt: EmbeddingMetadataMatrixBuilder):
    req = JOIN_COL_ORIGINAL, JOIN_COL_MIRROR, "keep_remove_label"
    for c in req:
        if c not in df.columns:
            raise KeyError(c)
    x_mat, feat_names = mt.transform(df)
...
def main(
    ...
    embedding_lookup = load_embeddings_via_dynamodb_and_s3(
        df,
        bucket=s3_bucket,
    ...
    prep = EmbeddingMetadataMatrixBuilder().fit(train_df)
    ...
    metrics_payload = {"train_metrics": train_metrics, "test_metrics": test_metrics}
    ...
    metadata = {
        ...
        "embedding_source": {
            ...
            "ddb_pointer_schema": (
                "Dynamo embedding_id → S3 JSON containing Bedrock-compatible payload vectors"
            ),
        },
        "feature_set_description": (
            "Frozen Bedrock Titan embeddings (original + mirror) with abs-diff/Hadamard/cosine features "
            "plus OneHot-encoded sampled stance/toxicity (encoder fit on train subset only)."
        ),

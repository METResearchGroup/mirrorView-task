...
class TestAppendPartyRows:
    ...
    def test_clones_are_last_in_each_party_list(self) -> None:
        ...
        rows = [source_row(user_id, mixed_post_ids(user_id)) for user_id in range(1, 5)]
        ...
        expected_first_posts = source_row(1, mixed_post_ids(1)).assigned_post_ids
    ...
    def test_require_original_party_prefix_raises_on_mismatch(self) -> None:
        ...
        original_rows = [
            source_row(user_id, mixed_post_ids(user_id)) for user_id in range(1, 3)
        ]
        expanded_rows = [
            source_row(user_id, mixed_post_ids(user_id)) for user_id in range(1, 5)
        ]

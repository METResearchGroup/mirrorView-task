# V1 Bedrock data prep — progress updates

Scope: **V1.1** analysis table + **V1.2** shared split only. No training / 2D viz / Bedrock re-runs.
...
- Artifacts (under `outputs/v1_bedrock/`):
...
- Convenience join: `outputs/v1_bedrock/analysis_with_split.csv` (`post_id,label,is_correct,is_error,split`)
...
- None for V1.1/V1.2. Worktree has no local `embedding_cache/`; used main-repo cache (gitignored, 17,370 `.npy` files). 100% local hits.

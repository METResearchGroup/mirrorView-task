"""Train XGBoost using original post embedding only.

Run from repo root::

    PYTHONPATH=. uv run python experiments/predict_keep_remove_2026_07_01/models/xgboost/train_only_original_post_embedding.py
"""
...
from experiments.predict_keep_remove_2026_07_01.features_only_original_embedding import (
...
def main(
    ...
    out_dir = _RUN_ROOT / "only_original_post_embedding" / timestamp
    ...
    print("Building fixed (original-only) feature matrix...")
    prep = OnlyOriginalEmbeddingFeatureBuilder().fit(train_df)
    ...
    if len(fi) != len(feature_names_fit):
        raise RuntimeError(
            f"Feature importance length mismatch: fi={len(fi)} names={len(feature_names_fit)}"
        )
    ...
    metadata = {
        ...
        "feature_set_description": (
            "Original-only fixed dense features: orig_emb(d) where orig_emb is the "
            "Bedrock Titan embedding vector (d dims)."
        ),

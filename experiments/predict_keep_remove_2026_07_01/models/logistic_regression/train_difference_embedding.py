"""Train logistic regression using (orig_emb - mirror_emb) only.

Run from repo root::

    PYTHONPATH=. uv run python experiments/predict_keep_remove_2026_07_01/models/logistic_regression/train_difference_embedding.py
"""
...
def main(
    ...
    out_dir = _RUN_ROOT / "difference_embedding" / timestamp
    ...
    print("Building fixed (difference-only) feature matrix...")
    prep = DifferenceEmbeddingFeatureBuilder().fit(train_df)
    ...
    metadata = {
        ...
        "feature_set_description": (
            "Difference-only fixed dense features: (orig_emb - mirror_emb) where each is the "

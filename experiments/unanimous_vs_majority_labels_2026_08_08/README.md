...
The analysis set is linked-fate trials with at least three raters. Posts fall into four cohorts, with the expected sizes on the current data shown below.

| Cohort | Expected n (current data) |
| ------ | ------------------------: |
...
Analysis 1 computes surface metrics and classifiers on original text, then summarizes them by cohort. The locked list is:
...
3. Count each remaining token at most once per post inside each cohort.
...
- `outputs/analysis2/wordcloud_<cohort>.png` for each of the four cohorts

## Analysis 3

Analysis 3 builds three tables of stance by cohort, one table for each toxicity stratum. Each table crosses `sampled_stance` (left or right) with the four cohorts.

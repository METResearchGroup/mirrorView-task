...
"""
Truncate original and mirrored posts with boundary-cascade + pair-aware v2 logic.
...
from experiments.truncate_posts_2026_06_19.truncation_v2 import (
    MAX_CHARS,
    MIN_KEEP_RATIO,
    truncate_pair,
)
...
TRUNCATION_VERSION = TruncationVersion.v2
...
def _catastrophic_truncation_count(
    original_before: pd.Series,
    mirrored_before: pd.Series,
    original_after: pd.Series,
    mirrored_after: pd.Series,
    *,
    max_chars: int,
) -> tuple[int, int]:
    """Posts where truncated length is below 30% of min(raw length, max_chars)."""
    orig_floor = original_before.clip(upper=max_chars) * 0.3
    mirr_floor = mirrored_before.clip(upper=max_chars) * 0.3
    orig_bad = original_after < orig_floor
    mirr_bad = mirrored_after < mirr_floor
    return int(orig_bad.sum()), int(mirr_bad.sum())
...
def build_truncated_df_v2(
    ...
    truncated = df.copy()
    truncated_pairs = [
        truncate_pair(original, mirror, MAX_CHARS, min_keep_ratio=MIN_KEEP_RATIO)

"""Intergroup-label analysis on linked MirrorView run data.
...
from experiments.mirrors_content_analysis_2026_04_24.analysis.intergroup_classifier.link_mirrorview_run_to_labels import (
    get_mirrorview_run_data_with_labels,
)
from lib.timestamp_utils import get_current_timestamp

LABEL_ORIGINAL_COL = "intergroup_clf_label_original_text"
LABEL_MIRROR_COL = "intergroup_clf_label_mirrors"
INTERGROUP_CLASSIFIER_DIR = Path(__file__).resolve().parent


class IntergroupLabelAnalyzer:
    """Analyze intergroup classifier labels attached to MirrorView run data."""

    def __init__(self, df: pd.DataFrame) -> None:
        self.df = df.copy()
        self.timestamp = get_current_timestamp()
        self.output_dir = INTERGROUP_CLASSIFIER_DIR / "outputs" / self.timestamp
    ...
    def keep_remove_analysis(self) -> None:
        """Intergroup-label rates split by keep/remove decision and key partitions."""
    ...
    def show_results(self) -> None:
        ...
        print("\n=== Original text intergroup label rates ===")
        print(self.results["original_text_analysis"])

        print("\n=== Mirror text intergroup label rates ===")
    ...
    def visualize_results(self) -> None:
        ...
        if isinstance(original, dict) and isinstance(mirror, dict):
            ...
            ax.set_ylim(0.0, 1.0)
            ax.set_ylabel("Intergroup rate")
            ax.set_title("Overall intergroup rate")
            fig.tight_layout()
        ...
        if isinstance(keep_remove, pd.DataFrame) and not keep_remove.empty:
            ...
            ax.set_ylim(0.0, 1.0)
            ax.set_ylabel("Intergroup rate")
            ax.set_title("Keep vs remove intergroup rates")
            ax.legend()
            fig.tight_layout()
...
def main() -> None:
    ...
    analyzer = IntergroupLabelAnalyzer(df)

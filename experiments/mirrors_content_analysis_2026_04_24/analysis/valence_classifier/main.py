"""Valence-label analysis on linked MirrorView run data.
...
from experiments.mirrors_content_analysis_2026_04_24.analysis.valence_classifier.link_mirrorview_run_to_labels import (
    get_mirrorview_run_data_with_labels,
)
from lib.timestamp_utils import get_current_timestamp

LABEL_ORIGINAL_COL = "valence_clf_label_original_text"
LABEL_MIRROR_COL = "valence_clf_label_mirrors"
VALENCE_CLASSIFIER_DIR = Path(__file__).resolve().parent


class ValenceLabelAnalyzer:
    """Analyze valence classifier labels attached to MirrorView run data."""

    def __init__(self, df: pd.DataFrame) -> None:
        self.df = df.copy()
        self.timestamp = get_current_timestamp()
        self.output_dir = VALENCE_CLASSIFIER_DIR / "outputs" / self.timestamp
    ...
    def keep_remove_analysis(self) -> None:
        """Positive-label rates split by keep/remove decision and key partitions."""
    ...
    def show_results(self) -> None:
        ...
        print("\n=== Original text valence label rates ===")
        print(self.results["original_text_analysis"])

        print("\n=== Mirror text valence label rates ===")
    ...
    def visualize_results(self) -> None:
        ...
        if isinstance(original, dict) and isinstance(mirror, dict):
            ...
            ax.set_title("Overall valence positive rate")
            fig.tight_layout()
        ...
        if isinstance(keep_remove, pd.DataFrame) and not keep_remove.empty:
            ...
            ax.set_ylim(0.0, 1.0)
            ax.set_ylabel("Positive rate")
            ax.set_title("Keep vs remove positive rates")
            ax.legend()
            fig.tight_layout()
...
def main() -> None:
    ...
    analyzer = ValenceLabelAnalyzer(df)

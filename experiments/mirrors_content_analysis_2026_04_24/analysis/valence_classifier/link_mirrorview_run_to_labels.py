"""Join valence-classifier labels onto transformed MirrorView run data."""
...
def get_mirrorview_run_data_with_labels() -> pd.DataFrame:
    """Attach valence labels to the latest transformed MirrorView trial table."""
    ...
    if "post_id" not in df.columns:
        raise KeyError(
            "Transformed MirrorView data must include 'post_id' to join valence labels."
        )
    ...
    for lbl_df, path in (
        ...
        if "post_primary_key" not in lbl_df.columns or "is_positive" not in lbl_df.columns:
            raise KeyError(
                f"Expected columns 'post_primary_key' and 'is_positive' in {path}"
            )

    orig_join = labels_original_text[["post_primary_key", "is_positive"]].rename(
        columns={"is_positive": "valence_clf_label_original_text"}
    )
    mir_join = labels_mirrors[["post_primary_key", "is_positive"]].rename(
        columns={"is_positive": "valence_clf_label_mirrors"}
    )

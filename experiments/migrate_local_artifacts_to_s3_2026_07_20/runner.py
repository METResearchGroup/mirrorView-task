...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        for prefix in sorted(by_prefix):
            c = by_prefix[prefix]
            print(f"prefix={prefix}  pending={c['pending']} skipped={c['skipped']}")
        ...
        try:
            ...
            tracker.close()
        by_prefix = defaultdict(lambda: {"pending": 0, "skipped": 0})
        for f in files:
            by_prefix[f.experiment_prefix][f.status] += 1
        for prefix in sorted(by_prefix):
            c = by_prefix[prefix]
            print(f"prefix={prefix}  pending={c['pending']} skipped={c['skipped']}")
        print(
            f"TOTAL pending={sum(1 for f in files if f.status == MigrationStatus.PENDING)} "
            f"skipped={sum(1 for f in files if f.status == MigrationStatus.SKIPPED)} "
...
def cmd_upload(args: argparse.Namespace) -> int:
    if not args.db.exists() and not args.dry_run:
        # Still allow dry-run only if DB exists for selecting rows; require DB always
        pass
        ...
        if args.check_stale:
            ...
            for r in rows:
                ...
                if size != int(r["file_size_bytes"]):
                    stale.append((r["local_path"], f"size {size} != db {r['file_size_bytes']}"))
                    continue
                if r.get("sha256"):

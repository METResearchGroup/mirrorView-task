...
def render_findings(
    ...
    return (
        ...
        f"Model: `{MODEL_ID}`\n\n"
        f"Region: `{BEDROCK_REGION}`\n\n"
        "Credentials: environment AWS keys (`LAB_AWS_ACCESS_KEY_ID` copied onto "
        "`AWS_ACCESS_KEY_ID` when needed).\n\n"
        "No IAM service role was created.\n\n"
        "## Converse\n\n"
        f"Probe text: {PROBE_USER_TEXT}\n\n"
        f"Model text: {converse['text']}\n\n"
        f"Input tokens: {converse['input_tokens']}\n\n"
        f"Output tokens: {converse['output_tokens']}\n\n"
        f"Stop reason: {converse['stop_reason']}\n\n"
        ...
        "## Native batch jobs\n\n"
        f"List jobs succeeded: {list_jobs['ok']}. "
        f"Listed job count: {list_jobs['listed_job_count']}.\n\n"
        ...
        "The live labeling path is Converse on-demand. Native batch jobs need "
        "`iam:PassRole` and a Bedrock service role, which this work must not create.\n"
    )

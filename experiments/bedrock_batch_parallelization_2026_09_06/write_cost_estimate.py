...
def render_cost_estimate(metrics: dict) -> str:
    ...
    return (
        ...
        f"The model is `{DEFAULT_BEDROCK_NOVA_MICRO}`.\n\n"
        "On-demand Ohio rates are "
        ...
        "Mean tokens per request from the smoke run of 100 posts are "
        f"{mean_input:.2f} input tokens and {mean_output:.2f} output tokens.\n\n"
        f"The actual cost of the smoke run of 100 posts was ${smoke_usd:.4f}.\n\n"
        f"The size runs total {size_posts:,} posts, and the estimated cost is "
        f"${size_usd:.4f}, from {size_input:,} input tokens and "
        f"{size_output:,} output tokens.\n\n"
        f"The process count runs total {process_posts:,} posts, and the estimated "
        f"cost is ${process_usd:.4f}, from {process_input:,} input tokens and "
        f"{process_output:,} output tokens.\n\n"
        f"The combined estimate is ${combined_usd:.4f}.\n\n"
        f"The allowed ceiling is twice the combined estimate, which is "
        f"${ceiling_usd:.4f}.\n\n"
